...
: ServoBase(
    node_data, {},
    ServoData(
      servo_data->pin, servo_data->min_pulse, servo_data->max_pulse, servo_data->min_angle,
...
bool PCAServo::set_angle_us(uint16_t duty_cycle)
{
  return pca9685_mod->set_mircoseconds(
    servo_data->pin,
    std::clamp(duty_cycle, (uint16_t)servo_data->min_pulse, (uint16_t)servo_data->max_pulse));
}