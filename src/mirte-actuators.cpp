...
Mirte_Actuator::Mirte_Actuator(TMX &tmx, ros::NodeHandle &nh,
                               Mirte_Board &board, std::vector<uint8_t> pins,
                               std::string name) {
  this->tmx = &tmx;
  this->nh = &nh;
  this->board = &board;
...
Motor::Motor(TMX &tmx, ros::NodeHandle &nh, Mirte_Board &board,
    ...
    : Mirte_Actuator(tmx, nh, board, pins, name) {
  motor_service = nh.advertiseService("/mirte/set_" + this->name + "_speed",
                                      &Motor::service_callback, this);
  ros_client = nh.subscribe("/mirte/motor_" + this->name + "_speed", 1000,
                            &Motor::motor_callback, this);
}
