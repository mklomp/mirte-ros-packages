...
public:
  ...
  Mirte_Sensor(TMX &tmx, ros::NodeHandle &nh, Mirte_Board &board,
               std::vector<uint8_t> pins, std::string name);
};

class KeypadMonitor : public Mirte_Sensor {
public:
  KeypadMonitor(TMX &tmx, ros::NodeHandle &nh, Mirte_Board &board,
                std::vector<uint8_t> pins, std::string name);
  void publish();
  ros::Publisher keypad_pub;
  ros::Publisher keypad_pressed_pub;
  ros::ServiceServer keypad_service;
  bool keypad_callback(mirte_msgs::GetKeypad::Request &req,
  ...
  get_keypad_monitors(ros::NodeHandle &nh, TMX &tmx, Mirte_Board &board);
};