...
class Mirte_Board_pcb : public Mirte_Board {
  public:
    ...
    std::map<std::string, int> resolveConnector(std::string connector) override;
    int resolvePin(std::string pin) override;
    virtual const double get_voltage_level() const override { return mcu->get_voltage_level(); }
    ...
    virtual const bool is_pwm_pin(uint8_t pin) const override { return mcu->is_pwm_pin(pin); };
