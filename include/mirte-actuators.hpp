...
public:
    ...
    Mirte_Actuator(TMX &tmx, ros::NodeHandle &nh, Mirte_Board &board, std::vector<uint8_t> pins, std::string name);
};

class Motor : public Mirte_Actuator
{
public:
    Motor(TMX &tmx, ros::NodeHandle &nh, Mirte_Board &board, std::vector<uint8_t> pins, std::string name);
    ros::ServiceServer motor_service;
    bool motor_callback(mirte_msgs::SetMotorSpeed::Request &req, mirte_msgs::SetMotorSpeed::Response &res);
    int last_speed = 0;
